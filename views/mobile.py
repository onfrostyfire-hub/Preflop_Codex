...
+    hero_pos = cfg.hero
+    villain_pos = cfg.villain
+    if sp == "EP vs 3bet Blinds":
+        villain_pos = random.choice(["SB", "BB"])
+
+    hero_idx = POSITIONS.index(hero_pos)
+    rot = POSITIONS[hero_idx:] + POSITIONS[:hero_idx]
+    is_defense_spot = villain_pos is not None
+
...
+        if is_defense_spot:
+            if p == villain_pos:
+                is_act = True; c_type = "3bet"
+            elif p in ["SB", "BB"]:
+                c_type = "blind"
+        else:
+            if POSITIONS.index(p) > POSITIONS.index(rot[0]) or (rot[0] == "SB" and p == "BB"):
+                is_act = True
+            if p in ["SB", "BB"]:
+                c_type = "blind"
